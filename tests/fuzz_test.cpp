...
void random_op(int user_idx) {
    ...
    if (do_delete) {
        ...
        // Broadcast
        if (target.clock != 0) {
            network_buffer.push_back({user_idx, Atom(), true, target});
        }
    } else {
        // Insert random char
        ...
        // Broadcast
        network_buffer.push_back({user_idx, new_atom, false, {0,0}});
    }
}
...
int main() {
    ...
    for(int u=0; u<NUM_USERS; u++) {
        for(const auto& packet : network_buffer) {
            // Self-check: Don't merge your own ops again (optional, CRDTs support idempotency usually, but our naive impl might split)
            // Actually, RGA remoteMerge is safe, but localInsert already applied it.
            // So we MUST skip packets from self to avoid duplication if the implementation doesn't check "Have I seen this ID?"
            // Our Sequence::remoteMerge DOES NOT check if ID exists. It blindly inserts.
            // Phase 2 Fix: Add Idempotency Check (Has OpID been seen?).
            
            // For now, simulator skips self-packets.
            if (packet.from_user == (u + 1)) continue;

            if (packet.is_delete) {
                users[u].remoteDelete(packet.delete_target);
            } else {
                users[u].remoteMerge(packet.atom);
            }
        }
    }
    ...
    for(int u=1; u<NUM_USERS; u++) {
        ...
        if (check != golden) {
            ...
            // Don't print strings, they are huge random garbage.
        }
    }
